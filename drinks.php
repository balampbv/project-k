<html>
<head><title>Drinks update</title></head>
...
if(isset($_POST['save']))
{
  ...
  if($image_type =='image/jpeg' || $image_type == 'image/gif'  || $image_type =='image/png')
  {
     ...
     //insert to database
    $query= mysql_query("INSERT INTO menu_drinks(item,name,size,cost,variety) values('$image','$image_description','$image_size','$cost','Drinks')");
	 if($uploadfile && $query)
     {
		 echo "<script>
...
Variety :
    <input type="text" name="variety" value="Drinks" disabled>
   File:
   ...
   <table>
     <thead>
       <tr>
		...
		<tbody>
		        <?php
			      $query=mysql_query ("select * from menu_drinks");
				  $i=1;
