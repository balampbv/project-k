<html>
<head><title>Starters update</title></head>
...
//  $variety=$_POST['variety'];
  ...
  if($image_type =='image/jpeg' || $image_type == 'image/gif'  || $image_type =='image/png')
  {
     ...
     //insert to database
     $query= mysql_query("INSERT INTO menu_starters(item,name,size,cost,variety) values('$image','$image_description','$image_size','$cost','Starters')");
	 if($uploadfile && $query)
     {
		 echo "<script>
...
Variety :
    <input type="text" name="variety" value="Starters" disabled>
	File:
   ...
   <table>
     <thead>
       <tr>
		...
		<tbody>
		        <?php
			      $query=mysql_query ("select * from menu_starters");
				  $i=1;
