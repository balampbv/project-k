<html>
<head><title>Main Course update</title></head>
...
if(isset($_POST['save']))
{
  ...
  if($image_type =='image/jpeg' || $image_type == 'image/gif'  || $image_type =='image/png')
  {
     ...
     //insert to database
	 $query= mysql_query("INSERT INTO menu_main(item,name,size,cost,variety) values('$image','$image_description','$image_size','$cost','$Main Course')");
	 if($uploadfile && $query)
     {
		 echo "<script>
...
Variety :
    <input type="text" name="variety" value="main" disabled>
   ...
   <table>
     <thead>
       <tr>
		...
		<tbody>
		        <?php
			      $query=mysql_query ("select * from menu_main");
				  $i=1;
